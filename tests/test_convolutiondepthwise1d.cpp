...
static int test_convolutiondepthwise1d_batch(int w, int h, int channels, int outh, int kernel, int dilation, int stride, int pad, int bias, int group)
{
    ncnn::Mat a = RandomMat(w, h, channels);
    ...
    if (ret != 0)
    {
        fprintf(stderr, "test_convolutiondepthwise1d_batch failed w=%d h=%d channels=%d outh=%d kernel=%d dilation=%d stride=%d pad=%d bias=%d group=%d act=%d actparams=[%f,%f]\n", w, h, channels, outh, kernel, dilation, stride, pad, bias, group, activation_type, activation_params[0], activation_params[1]);
    }
...
static int test_convolutiondepthwise1d_2()
{
    return 0
           || test_convolutiondepthwise1d_batch(25, 12, 2, 24, 3, 1, 1, 1, 1, 1)
           || test_convolutiondepthwise1d_batch(25, 12, 3, 12, 3, 1, 2, -233, 0, 3)
           || test_convolutiondepthwise1d_batch(31, 8, 5, 8, 5, 2, 1, 1, 1, 8)
           || test_convolutiondepthwise1d_batch(15, 16, 5, 24, 2, 1, 2, 0, 0, 8);
}
...
int main()
{
    ...
    return test_convolutiondepthwise1d_0() || test_convolutiondepthwise1d_1() || test_convolutiondepthwise1d_2();
}
